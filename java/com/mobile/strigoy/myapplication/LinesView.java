...
/**
 * LinesView contain multitouch interface and display multiple lines
 */
public class LinesView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(style ==1)if(styleSet){setBackgroundColor(Color.WHITE);
            styleSet =false;}
        if(style ==3)if(styleSet){setBackgroundColor(Color.BLACK);
            styleSet =false;}
        max_width =getWidth();
        ...
        for(int i1 = 0; i1< line_count; i1++)for(int i = 0; i< line[i1].pointCount +1; i++) {
            setColor();
        ...
        if(animatiOn){ //long time = SystemClock.uptimeMillis() % 100L; 4000L
            ...
            //instr_a+=5;
            animationVar += animationVar1;if(animationVar ==140.f|| animationVar ==0.f){
                animationVar1 *=-1.f;}
            invalidate();
        }
        color.defaultRainbow();
